...
```
CLI (cli.py)
  → resolve corpus, poison docs, queries (from profile or custom paths)
  → require_rxp_deps()          guard chromadb + sentence_transformers
  → validate_retrieval()         orchestrator (validator.py)
      → resolve_model()          registry shortcut or ad-hoc HuggingFace name
      → get_embedder()           SentenceTransformer, process-cached
      → RetrievalCollection()    ephemeral ChromaDB + UUID-suffixed collection
          → ingest()             encode docs via Embedder, add to ChromaDB
          → query()              encode query, nearest-neighbor search
      → QueryResult              per-query: hits, poison_retrieved, poison_rank
      → ValidationResult         aggregate: retrieval_rate, mean_poison_rank
  → print results / write JSON
```
